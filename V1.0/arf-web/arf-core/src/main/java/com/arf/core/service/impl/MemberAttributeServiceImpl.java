...
import com.arf.core.dao.MemberAttributeDao;
import com.arf.core.dao.MemberDao;
import com.arf.core.entity.Area;
import com.arf.core.entity.Member;
import com.arf.core.entity.Member.Gender;
...
public class MemberAttributeServiceImpl extends BaseServiceImpl<MemberAttribute, Long> implements MemberAttributeService {
	...
	@Override
	protected BaseDao<MemberAttribute, Long> getBaseDao() {
	...
	public boolean isValid(MemberAttribute memberAttribute, String[] values) {
		...
		case gender:
			try {
				Member.Gender gender = Member.Gender.valueOf(value);
		...
		case birth:
			try {
				Date birth = DateUtils.parseDate(value, CommonAttributes.DATE_PATTERNS);
		...
		case select:
			...
			if (CollectionUtils.isEmpty(memberAttribute.getOptions()) || !memberAttribute.getOptions().contains(value)) {
				return false;
			}
			break;
		case checkbox:
			...
			if (CollectionUtils.isEmpty(memberAttribute.getOptions()) || !memberAttribute.getOptions().containsAll(Arrays.asList(values))) {
				return false;
			}
			break;
		}
	...
	public Object toMemberAttributeValue(MemberAttribute memberAttribute, String[] values) {
		...
		case area:
			Long id = NumberUtils.toLong(value, -1L);
			return id != -1L ? areaDao.find(id) : null;
		case select:

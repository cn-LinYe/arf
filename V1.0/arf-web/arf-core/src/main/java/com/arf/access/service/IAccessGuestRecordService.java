...
public interface IAccessGuestRecordService extends
		BaseService<AccessGuestRecord, Long> {
	 ...
	 * @return
	 */
	AccessGuestRecord findById(Long id,Status status);
	 ...
	 * @param applytimeouthour (单位：小时) 申请时间(applyDate)与当前时间(now)的差值超过此值，即为超时
	 */
